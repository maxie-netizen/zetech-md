...
let trashplug = async (m, { trashown,text,conn,reply,isAdmins,prefix, command }) => {
				...
				await conn.groupParticipantsUpdate(m.chat, [users], 'demote').then((res) => reply(mess.succes)).catch((err) => reply(mess.error))
			};
