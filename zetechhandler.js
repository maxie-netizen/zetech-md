...
case 'groups': case 'mychats': case 'listgroups': {
    ...
    try {
        // Try to get chats from the connection
        let chats = [];
        
        // Method 1: Try getChats if available
        if (typeof conn.getChats === 'function') {
            try {
                chats = await conn.getChats();
            } catch (e) {
                console.log('[GROUPS] getChats() failed');
            }
        }
        
        // Method 2: Try store.chats
        if ((!chats || chats.length === 0) && conn.store && conn.store.chats) {
            try {
                chats = Object.values(conn.store.chats.all());
            } catch (e) {
                console.log('[GROUPS] Store method failed');
            }
        }
        
        // Method 3: Try direct conn.chats access
        if ((!chats || chats.length === 0) && conn.chats) {
            try {
                chats = Object.values(conn.chats);
            } catch (e) {
                console.log('[GROUPS] Direct access failed');
            }
        }
        
        console.log(`[GROUPS] Found ${chats ? chats.length : 0} total chats`);
